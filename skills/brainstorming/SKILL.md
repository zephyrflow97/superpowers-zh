---
name: brainstorming
description: "当需要构建新功能、新组件、新子系统，或改动会跨多个文件/影响架构/存在多种合理方案需要权衡时使用。单文件局部修复、重命名、配置微调、文档变更等边界明确的小改动不需要。"
---
...
**必须进入 brainstorming 的情况：**

- 构建新功能、新组件、新子系统，或创建新项目
- 改动会跨多个文件/模块，或影响架构、数据模型、公共 API
- 需求不清晰，或存在多种合理方案需要权衡
- 用户只描述了"想要什么"，没描述"怎么做"，且"怎么做"并不明显

**可以直接跳过 brainstorming 的情况：**

- 单文件局部的 bug 修复，且根因已明确（或通过 systematic-debugging 已定位）
- 重命名、类型调整、格式化、lint 修复
- 配置微调、依赖升级、环境变量改动
- 文档、注释、翻译类改动
- 用户已明确给出实现方式的指令（"把 X 改成 Y"、"在这一行加一个日志"）
- 探索性地读代码、回答问题、解释代码

**判断标准：** 当改动同时具备**明确的边界**、**明确的实现路径**、**明确的验收方式**这三点时，跳过 brainstorming，直接调用对应的实现/修复技能。只要有一项不明确，就进入 brainstorming。

**注意：** 即使跳过了 brainstorming，完成前的 verification-before-completion、test-driven-development、systematic-debugging 等质量闸门仍然适用——跳过的是"想清楚要做什么"，不是"做完要检查"。

## 反模式：对真正需要设计的工作说"这太简单了"

对于满足上方"必须进入"条件的工作，不要以"简单"为由跳过。"感觉简单"的新功能恰恰是未经检验的假设造成最多浪费的地方。设计可以很简短（几句话到一段都行），但必须展示出来并获得批准。
...
**终止状态是调用 writing-plans。** 不要调用任何实现技能。头脑风暴之后你唯一要调用的技能是 writing-plans。
